...
- **Thumbprint.** `github_actions_thumbprints` lists the SHA-1 thumbprints of GitHub's OIDC **token-signing certificates** (AWS accepts up to 5 — keep old + current across a rotation). GitHub rotated its signing keys in 2026; if OIDC fails with `Not authorized to perform sts:AssumeRoleWithWebIdentity`, pull the current signing-cert thumbprints from the JWKS (NOT the TLS leaf): `curl -s https://token.actions.githubusercontent.com/.well-known/jwks | jq -r '.keys[].x5c[0]' | while read c; do echo "$c" | base64 -d | openssl x509 -inform DER -fingerprint -noout -sha1; done`, add any new ones, keep the old.
